...
import { readConfigFromDisk, maskSecretKey } from '../../../config/index.js';
...
function maskSecrets(configObj) {
  ...
  if (Array.isArray(masked.parsedTableConfigs)) {
    masked.parsedTableConfigs = masked.parsedTableConfigs.map(c => ({
      ...c,
      appSecret: maskSecretKey(c.appSecret)
    }));
  }
  if (masked.defaultWpsConf && masked.defaultWpsConf.appSecret) {
    masked.defaultWpsConf.appSecret = maskSecretKey(masked.defaultWpsConf.appSecret);
  }
  if (masked.defaultFeishuConf && masked.defaultFeishuConf.appSecret) {
    masked.defaultFeishuConf.appSecret = maskSecretKey(masked.defaultFeishuConf.appSecret);
  }
